...
const needle = { angle: -120, velocity: 0 };
let targetAngle = -120;
let accent = "#2f6f4e";
...
function stepNeedle(dt) {
  ...
  needle.velocity += acceleration * dt;
...
function drawFace() {
  ...
  ctx.lineTo(cx + Math.cos(needleRad + Math.PI) * backR, cy + Math.sin(needleRad + Math.PI) * backR);
...
function applyUsage(payload) {
  if (!payload?.ok) {
    errorText = payload?.error || "Unable to read usage";
    labelEl.textContent = "!";
    subtitleEl.textContent = "Sign in?";
    accent = "#c23b22";
    targetAngle = -120;
    return;
  }
  ...
  if (payload.isUnlimited) {
    labelEl.textContent = "∞";
    subtitleEl.textContent = payload.membershipType || "Unlimited";
    accent = "#2f6f4e";
    targetAngle = -120;
    return;
  }
  ...
  subtitleEl.textContent = payload.membershipType || "Cursor";
  accent = colorForPercent(percent);
  const clamped = Math.max(0, Math.min(percent, 100));
  targetAngle = -120 + (240 * clamped) / 100;
}

// Drag window from renderer
let dragging = false;
let lastX = 0;
let lastY = 0;
...
window.tokenMeter?.onUsageUpdate(applyUsage);
requestAnimationFrame(frame);
